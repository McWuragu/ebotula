...
QueueExeStatus pushQueue(PQueue pqueueIn, QueueData queuedataElement)
{
    ...
    if (!pqueueIn || !pqueueIn->sentinel || !pqueueIn->sentinel->queue_mutex ||
        !pqueueIn->sentinel->StopThreadCond)
    {
        return QUEUE_NULL_POINTER_AS_IN_PARAMETER;
    }

    if (queuedataElement.t_size > 0 && queuedataElement.data == NULL)
    {
        return QUEUE_NULL_POINTER_AS_IN_PARAMETER;
    }
    ...
    pqueueNew->queuedataData->data=NULL;
    if (queuedataElement.t_size > 0)
    {
        pqueueNew->queuedataData->data=malloc(queuedataElement.t_size);
        if (pqueueNew->queuedataData->data == NULL)
        {
            logger(LOG_ERR,_("Couldn't allocate memory!"));
            free(pqueueNew->queuedataData);
            free(pqueueNew);
            pthread_mutex_unlock(pqueueIn->sentinel->queue_mutex);
            return QUEUE_MEMORY_ALLOC_ERROR;
        }
...
QueueData * popQueue(PQueue pqueueIn)
{
    ...
    /* condition handling */
    while (pqueueIn->sentinel->longCount<=0) {
